...
### 0x1006 a/b color fade in

该命令和0x1002命令使用同样的代码，区别在于：

1. 0x1002命令早期设计，缺省时间单位是0.1s，实际并没有用途；本命令当前以秒为缺省单位，未来可根据设计要求扩充；
2. 0x1002命令是从当前颜色（即上一个命令执行到的颜色）过渡到该命令设置的颜色；本命令直接提供a/b两个颜色，设定为从a过渡到b。

本命令可以通过提供option的方式定义HSV1不是编码里提供的值，而是沿用收到命令时上一个命令运行到的值，如果提供该选项则本命令可完全替代0x1002命令。



| code (2 bytes) | option bits (1 bytes) | Fade in Duration (2 bytes) | HSV1 (3 bytes) | HSV2 (3 bytes) | Reserved (4 bytes) |
| -------------- | --------------------- | -------------------------- | -------------- | -------------- | ------------------ |
| `1006`         | `00`                  | `0000`                     | `hhssvv`       | `hhssvv`       | `00000000`         |



使用本命令可以实现渐近变亮和渐近变暗的业务要求；如果仅仅是亮度变化，**HSV1和HSV2应使用相同的HS，仅V不同**，这样可以保证在变化时色调一致。
...
```
# magic    seq groupId  bits  code opt duration hsv1   hsv2   padding
# b01bc0de 00  a5a5a5a5 ffff  1006 00  0005     00ff00 00ffff 00000000
```
...
```
# magic    seq groupId  bits  code opt duration hsv1   hsv2   padding
# b01bc0de 00  a5a5a5a5 ffff  1006 00  0005     00ffff 00ff00 00000000
